...
function generateSchemaSectionText(octothorpes, name, isRequired, schema, subSchemas) {
	...
	if (schema.additionalProperties) {
		if (schema.additionalProperties.type != 'string' && schema.additionalProperties.properties) {
			Object.keys(schema.additionalProperties.properties).forEach((key) => {
				props[key] = schema.additionalProperties.properties[key]
			})
		}
	}
	...
	if (schemaType === 'object') {
		...
		if (schema.additionalProperties && schema.additionalProperties.type == 'string') {
			text.push('Further properties are of type `' + schema.additionalProperties.type + '`')
		}
	...
	if (schema.default) {
		if ([ "boolean", "number", "string" ].indexOf(typeof schema.default) !== -1) {
...
module.exports = function(schema) {
	...
	text.push('# example')
	text.push('```json')
	text.push(JSON.stringify(schema.example, null, 2))
	text.push('```')
	text.push('# schema')
